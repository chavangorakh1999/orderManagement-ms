...
describe('Menu API', () => {
  describe('GET /api/menu', () => {
    ...
    it('returns all seeded menu items', async () => {
      await MenuItem.create(validMenuItem);
      await MenuItem.create({ ...validMenuItem, name: 'Test Burger', category: 'Burger' });
      const res = await request(app).get('/api/menu');
      expect(res.status).toBe(200);
  ...
  describe('GET /api/menu/:id', () => {
    it('returns a single menu item by ID', async () => {
      const created = await MenuItem.create(validMenuItem);
  ...
  describe('POST /api/menu', () => {
    ...
    it('returns 400 when price is negative', async () => {
      const res = await request(app).post('/api/menu').send({ ...validMenuItem, price: -5 });
      expect(res.status).toBe(400);
      expect(res.body.error.code).toBe('VALIDATION_ERROR');
    });

    it('returns 400 when price is not a number', async () => {
      const res = await request(app).post('/api/menu').send({ ...validMenuItem, price: 'free' });
      expect(res.status).toBe(400);
      expect(res.body.error.code).toBe('VALIDATION_ERROR');
    });

    it('returns 400 when category is invalid', async () => {
      const res = await request(app).post('/api/menu').send({ ...validMenuItem, category: 'InvalidCategory' });
      expect(res.status).toBe(400);
      expect(res.body.error.code).toBe('VALIDATION_ERROR');
    });

    it('returns 400 when description is too short', async () => {
      const res = await request(app).post('/api/menu').send({ ...validMenuItem, description: 'Short' });
      expect(res.status).toBe(400);
      expect(res.body.error.code).toBe('VALIDATION_ERROR');
    });
  });
  ...
  describe('PUT /api/menu/:id', () => {
    it('updates an existing menu item', async () => {
      const created = await MenuItem.create(validMenuItem);
      const res = await request(app).put('/api/menu/' + created.id).send({ name: 'Updated Pizza', price: 15.99 });
      expect(res.status).toBe(200);
    ...
    it('returns 404 for non-existent ID', async () => {
      const res = await request(app).put('/api/menu/000000000000000000000000').send({ name: 'Updated' });
      expect(res.status).toBe(404);
      expect(res.body.error.code).toBe('NOT_FOUND');
    });
    ...
    it('returns 400 for invalid update data', async () => {
      const created = await MenuItem.create(validMenuItem);
      const res = await request(app).put('/api/menu/' + created.id).send({ price: -10 });
      expect(res.status).toBe(400);
      expect(res.body.error.code).toBe('VALIDATION_ERROR');
    });
  });
  ...
  describe('DELETE /api/menu/:id', () => {
    ...
    it('returns 404 for non-existent ID', async () => {
      const res = await request(app).delete('/api/menu/000000000000000000000000');
      expect(res.status).toBe(404);
      expect(res.body.error.code).toBe('NOT_FOUND');
    });
  });
